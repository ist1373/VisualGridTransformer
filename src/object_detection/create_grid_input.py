...
def create_grid(pdf, output, tokenizer, model):
    if not os.path.exists(output):
        os.makedirs(output)

    word_grid = return_word_grid(pdf)
    ...
    for page in range(len(word_grid)):
        ...
        if page==0:
            print(grid)
            print(len(grid['bbox_texts_list']))
            print(len(grid['texts']))
        save_pkl_file(grid, output, f"page_{page}", model)
